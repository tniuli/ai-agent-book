...
在进入更复杂的多工具场景之前，有一个容易踩坑的地方值得提前说明：当多个工具之间存在依赖关系时（后一个工具需要前一个的结果作为输入），必须串行调用，如果误用并发模式同时发起，依赖方会因为缺少前置数据而直接报错，甚至触发连锁超时——第一个工具响应慢了，所有依赖它的工具跟着一起超时。即使工具之间逻辑上相互独立，单个工具的超时也可能拖慢整个 Agent 的响应，让用户侧感觉"卡住了"。所以实际开发中要严格区分串行与并行场景，并为每个工具设置独立的超时阈值，超时后返回降级结果而非阻塞等待。
