...
## 18.1 为什么 Agent 调试这么难

Agent 调试之所以困难，根源在四个层面：非确定性、长链条、隐式状态和外部依赖。我们逐一分析。

### 18.1.1 非确定性：同样的输入，不同的输出

传统程序是确定性的——给定相同的输入，必定得到相同的输出。但 Agent 的核心是 LLM，LLM 本质上是一个概率模型。你问它同样的问题，它可能给出完全不同的回答，甚至选择不同的工具。
...
这里需要避开一个常见的实践误区：用 `print` 调试 Agent 几乎不可行。Agent 的多步执行意味着输出太多时找不到关键信息，输出太少又遗漏问题线索；另一个极端是记录所有内容（包括完整的 LLM 响应和工具返回），导致日志膨胀、成本飙升。合理的做法是采用三层日志架构——默认只记录请求级和步骤级，细节级按需开启；所有日志统一输出为 JSON 格式，方便后续搜索和聚合。
...
## 18.3 LangSmith trace 分析实战

LangSmith 是 LangChain 团队推出的 Agent 可观测性平台，它提供了 Trace 可视化和分析功能。即使你不使用 LangChain，LangSmith 也值得了解——因为它代表了 Agent 调试工具的设计范式。

### 18.3.1 LangSmith 的核心概念
...
### 18.4.2 LangGraph 的 Replay 能力

LangGraph 内置了 Replay 能力，这是它相比其他 Agent 框架的一个显著优势。
...
LangGraph 的 Replay 最有用的功能是"时光机"——你可以修改历史状态，然后从某个节点重新执行。
...
| 模型 | 输入价格 | 输出价格 | 备注 |
|------|---------|---------|------|
| GPT-5.5 | $2.50/1M | $10.00/1M | 2025 年价格 |
...
class CostConfig:
    ...
    prices: dict = field(default_factory=lambda: {
        ...
        "claude-sonnet-4-6": {"input": 0.15 / 1_000_000, "output": 0.60 / 1_000_000},
        "deepseek-v4-flash": {"input": 0.02 / 1_000_000, "output": 1.00 / 1_000_000},
    })
    ...
    def record(self, record: TokenRecord):
        ...
        if self.budget_limit:
            ...
            if total > self.budget_limit * 0.8:
                print(f"WARNING 成本告警：已使用 {total:.4f} 美元，"
...
这一节，我们将实现一个完整的 Agent 全链路追踪系统，包括日志记录、Trace 可视化、成本监控和 Replay 回放。
...
@tracer.trace_llm(model="claude-opus-4-7")
...
步骤详情:
  [1] LLM Call (claude-opus-4-7) - 850ms, 350 tokens in / 80 out
  [2] Tool Call (query_order) - 1,200ms
  [3] LLM Call (claude-opus-4-7) - 1,100ms, 500 tokens in / 150 out
  [4] Output Parser - 50ms

成本分布:
  claude-opus-4-7: $0.0085 (输入: $0.0021, 输出: $0.0064)
=====================================
```
