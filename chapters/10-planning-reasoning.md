...
不过 ToT 的实际使用中有两个需要留意的问题。一是 DFS 搜索容易陷入局部最优——Agent 可能沿着一条看似有希望的路径越走越深，但实际上已经偏离了正确方向。二是 LLM 作为状态评估器时打分不稳定——同一个思维状态，上一次评估给 8 分，下一次可能只给 5 分，导致搜索策略摇摆不定。实际开发中可以通过设置搜索深度上限和回溯阈值（比如连续 2 次评估下降就回溯）来缓解局部最优问题，对 LLM 评分做多次采样取平均来提升稳定性，或者在浅层用 BFS 充分探索后再考虑深入。
