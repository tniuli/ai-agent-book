...
```mermaid
flowchart TD
    Q["问题"] --> C1["CoT采样1 → 推理路径1 → 答案A"]
    Q --> C2["CoT采样2 → 推理路径2 → 答案B"]
    Q --> C3["CoT采样3 → 推理路径3 → 答案A"]
    Q --> C4["CoT采样4 → 推理路径4 → 答案A"]
    Q --> C5["CoT采样5 → 推理路径5 → 答案C"]
    C1 & C2 & C3 & C4 & C5 --> Vote["多数投票"]
    Vote --> Final["答案A ✓"]
```
...
不过树搜索也有自己的坑。使用 DFS 搜索思维树时，Agent 可能沿着一条看似有希望的路径越走越深，实际上已经偏离了正确方向——这就是局部最优陷阱。更棘手的是 LLM 作为状态评估器，其打分在不同调用之间可能不稳定：同一个思维状态，上一次评估给 8 分，下一次可能只给 5 分，导致搜索策略摇摆不定。实际应用中需要设置搜索深度上限和回溯阈值（连续 2 次评估下降则回溯），对 LLM 评分做多次采样取平均以提高稳定性，或者干脆用 BFS 在浅层充分探索后再深入。
...
flowchart LR
    subgraph CoT["CoT（线性推理）"]
        direction LR
        C1["思维1"] --> C2["思维2"] --> C3["思维3"] --> CA["答案"]
    end
    subgraph ToT["ToT（树状推理）"]
        direction LR
        T1["思维1"] --> T2a["思维2a"]
        T1 --> T2b["思维2b"]
        T2a --> T3a["思维3a ✓"]
...
```mermaid
flowchart TD
    Start["输入：1, 2, 3, 4<br/>目标：用 +, -, ×, ÷ 使结果为24"]
    Start --> S1["第1步：选择两个数运算"]
    S1 --> A1["1+2=3, 剩余 [3,3,4]<br/>评估：中等"]
    S1 --> A2["2×3=6, 剩余 [1,6,4]<br/>评估：高 ★"]
    S1 --> A3["3×4=12, 剩余 [1,2,12]<br/>评估：高 ★"]
    S1 --> A4["1×4=4, 剩余 [2,3,4]<br/>评估：中等"]
    A2 --> B1["[1,6,4] → 6×4=24<br/>剩余 [1,24] → 1×24=24 ✓"]
    A3 --> B2["[1,2,12] → 12×2=24<br/>剩余 [1,24] → 1×24=24 ✓"]
```
...
```mermaid
flowchart TD
    Task["用户任务"] --> Planner["规划器<br/>(Planner)"]
    Planner --> Executor["执行器<br/>(Executor)"]
    Executor --> Feedback["执行反馈"]
    Feedback --> Replanner["重规划器<br/>(Replanner)"]
    Replanner --> Planner
```
...
flowchart TD
    ...
    Replanner -->|"continue"| Executor2["Executor"]
    Replanner -->|"end"| END["END"]
```
...
flowchart TB
    subgraph RA["Research Agent"]
        direction LR
        Plan["Plan<br/>Node"] --> Exec["Execute<br/>Node"]
        Exec --> Refl["Reflect<br/>Node"]
        Refl --> Repl["Replan<br/>Node"]
        Exec --> Tools["Tools<br/>Search / Analyze / Write"]
        Repl --> Memory["记忆存储<br/>反思记录 / 执行历史"]
    end
```
...
```mermaid
flowchart TB
    subgraph Matrix["CoT 策略决策矩阵"]
        direction LR
        subgraph Row1["简单任务（1-2步推理）"]
            R1C1["✓ 首选：低成本<br/>快速可控"]
            R1C2["△ 可用但浪费<br/>成本高，杀鸡用牛刀"]
        end
        subgraph Row2["中等任务（3-5步推理）"]
            R2C1["✓ 可用：需要精心<br/>设计 CoT 模板"]
            R2C2["✓ 可用：自动推理<br/>无需额外提示词"]
        end
        subgraph Row3["复杂任务（5+步推理）"]
            R3C1["△ 勉强：容易出错<br/>缺乏纠错能力<br/>线性推理有瓶颈"]
            R3C2["✓ 首选：内置回溯<br/>自动纠错更可靠"]
        end
        subgraph Row4["结构化推理（需特定格式）"]
            R4C1["✓ 首选：可控框架<br/>透明可审计"]
            R4C2["△ 可用但不可控<br/>内部推理黑箱"]
        end
    end
    ColHeader1["普通模型 + CoT"] --- ColHeader2["推理模型（o1/o3）"]
```
...
flowchart LR
    subgraph CoT["CoT（线性推理）"]
        direction LR
        CQ["Q"] --> CS1["S1"] --> CS2["S2"] --> CA["A"]
        NoteC["单路径，不回溯"]
    end
    subgraph SC["自一致性（多路径投票）"]
        direction LR
        SQ["Q"] --> SS1["S1 → S2 → A1"]
        SQ --> SS2["S3 → S4 → A2"]
        SQ --> SS3["S5 → S6 → A1"]
        SS1 & SS2 & SS3 --> Vote["多数投票 → A1 ✓"]
        NoteS["多路径，事后投票"]
    end
    ...
    subgraph PE["Plan-and-Execute"]
        direction LR
        PPlan["Plan"] --> PExec1["Exec1"] --> PReplan["Replan"] --> PExec2["Exec2"] --> PResult["Result"]
        NoteP["先规划，后执行，可调整"]
    end
    subgraph Ref["Reflexion"]
        direction LR
        RExec1["Exec1"] --> REval["Eval"] --> RReflect["Reflect"] --> RExec2["Exec2"] --> RSuccess["Success"]
        REval -->|"失败"| RReflect
        NoteR["执行+评估+反思循环"]
    end
```
