...
| 规划器模型 | 强模型（如 Claude Opus 4.7） | 轻量模型 | 规划用强模型，执行用轻量模型 |
...
plan_chain = plan_prompt | ChatOpenAI(model="claude-sonnet-4-6", base_url=os.getenv("OPENAI_BASE_URL"))
...
execute_chain = execute_prompt | ChatOpenAI(model="claude-sonnet-4-6", base_url=os.getenv("OPENAI_BASE_URL"))
...
replan_chain = replan_prompt | ChatOpenAI(model="claude-sonnet-4-6", base_url=os.getenv("OPENAI_BASE_URL"))
...
# LLM 绑定工具
llm = ChatOpenAI(model="claude-sonnet-4-6", temperature=0, base_url=os.getenv("OPENAI_BASE_URL"))
...
**为什么用 Claude Sonnet 4.6 而不是 Claude Opus 4.7？**

研究型 Agent 的每次运行可能涉及10+次 LLM 调用（规划1次+执行N次+反思N次+重规划N次+报告1次）。用 Claude Opus 4.7 成本过高，Claude Sonnet 4.6 在推理任务上已经有不错的表现。如果需要更高质量的输出，可以对规划节点和报告生成节点使用更强的模型，执行节点仍用轻量模型——这正是 Plan-and-Execute 的优势：不同步骤可以用不同模型。
...
2. Yao, S. et al. "Tree of Thoughts: Deliberate Problem Solving with Large Language Models." NeurIPS 2023. arXiv:2305.10601
3. Shinn, N. et al. "Reflexion: Language Agents with Verbal Reinforcement Learning." NeurIPS 2023. arXiv:2303.11366
