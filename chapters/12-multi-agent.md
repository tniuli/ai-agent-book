...
> (A 向 Supervisor 汇报，Supervisor 决定下一步)
>
> - 特点：中心化控制，流程清晰
> - 适用：流水线任务、审批流程
> - 风险：Supervisor 成为瓶颈
...
```mermaid
flowchart TB
    TS["Top Supervisor"] --> SS1["Sub-Supervisor"]
    TS --> SS2["Sub-Supervisor"]
    SS1 --> A1["A1"]
    SS1 --> A2["A2"]
    SS1 --> A3["A3"]
    SS2 --> B1["B1"]
...
```mermaid
flowchart LR
    A["Agent A<br/>需求分析"] --> B["Agent B<br/>代码编写"] --> C["Agent C<br/>测试验证"]
```
...
```mermaid
flowchart TB
    Host["主持人<br/>提出问题"] --> Pos["正方A"]
    Host --> Neg["反方B"]
    Host --> Judge["评审C"]
    Pos <-->|"辩论"| Neg
    Neg -->|"评审"| Judge
    Pos --> Final["投票/综合 → 最终方案"]
    Neg --> Final
    Judge --> Final
```
...
```mermaid
flowchart TB
    Coord["协调者<br/>拆分任务"] --> PA["Agent 部分A"]
    Coord --> PB["Agent 部分B"]
    Coord --> PC["Agent 部分C"]
    PA --> Integrator["整合者<br/>合并产出"]
    PB --> Integrator
    PC --> Integrator
```
...
上一节讲了设计模式的理论，这一节我们看看 LangGraph 框架如何将这些模式落地。LangGraph 提供了三种官方推荐的多 Agent 编排模式：Supervisor、Swarm 和 Hierarchical。
...
```mermaid
flowchart TB
    S["Supervisor<br/>路由+协调"] --> A["Agent A"]
    S --> B["Agent B"]
    S --> C["Agent C"]
```
...
```mermaid
flowchart TB
    TS["Top Supervisor"] --> SS1["Sub-Supervisor<br/>研发团队"]
    TS --> SS2["Sub-Supervisor<br/>运维团队"]
    SS1 --> Dev1["Dev1"]
    SS1 --> Dev2["Dev2"]
    SS1 --> Tester["Tester"]
    SS2 --> Ops1["Ops1"]
...
> 每层 Supervisor 只管理自己的子 Agent，无需了解全局细节
...
我的建议是：**从 Supervisor 开始**。它是最简单、最可控的模式，适合 80% 的场景。当你发现 Supervisor 确实成为瓶颈时，再考虑 Swarm 或 Hierarchical。过早引入复杂架构，只会让系统更难调试和维护。
...
flowchart TB
    ...
    A1["Agent A<br/>(写)"] -->|"写 →"| State
    State -->|"读 →"| B1["Agent B<br/>(读)"]
    A2["Agent C<br/>(写)"] -->|"写 →"| State
    State -->|"读 →"| D1["Agent D<br/>(读)"]
```
...
```mermaid
flowchart LR
    A["Agent A<br/>inbox:[]"] -->|"message"| B["Agent B<br/>inbox:[msg]"]
    B -->|"message"| C["Agent C<br/>inbox:[msg]"]
```
...
flowchart TB
    ...
    AA["Agent A<br/>写需求"] -->|"写"| SS
    AC["Agent C<br/>写测试结果"] -->|"写"| SS
    AA -->|"message<br/>需求已就绪，请编码"| AC
```
...
一个常见的误区是过度追求"完美"的通信架构。实际上，对于大多数项目来说，LangGraph 的共享状态已经足够。当你真正遇到状态冲突或扩展瓶颈时，再引入消息传递机制也不迟。**先让它跑起来，再让它跑得好。**
...
```mermaid
flowchart TB
    subgraph Main["主 Agent (Deep Agent)"]
        direction TB
        SA["子Agent A<br/>独立上下文<br/>独立工具集"]
        SB["子Agent B<br/>独立上下文<br/>独立工具集"]
        SC["子Agent C<br/>独立上下文<br/>独立工具集"]
    end

    Main --- Skills["技能: data_collection, analysis, report"]
    Main --- FS["文件系统: 本地/沙箱/远程"]
    Main --- Mem["记忆: 跨会话持久化"]
```
...
```mermaid
flowchart TB
    LG["LangGraph<br/>图运行时（最底层）"] --> CA["create_agent<br/>轻量 Agent 框架（中间层）"]
    CA --> DA["Deep Agents<br/>全功能自主 Agent 框架（最高层）"]
```

> 三者共享同一套基础构建块，但每一层封装了更多能力。你可以根据需求选择合适的抽象层。
...
```mermaid
flowchart LR
    Collect["数据采集"] --> Clean["数据清洗"] --> Analyze["数据分析"] --> Report["报告生成"]
    Report --> Reflect["质量反思"]
    Reflect -->|"质量达标"| Done["结束"]
    Reflect -->|"质量不达标"| Analyze
```
...
这段代码的结构非常清晰：前四个节点是线性流水线，`reflect` 节点是分叉点——质量达标则结束，不达标则退回 `analyze` 重新分析。注意退回的是 `analyze` 而非 `collect`——如果数据本身没问题，只是分析不够深入，没必要重新采集数据。这种"退回最近的问题节点"的策略，比"从头再来"更高效。
...
TypeScript 的类型系统在多 Agent 开发中是一个巨大优势——你可以用接口精确描述每个 Agent 的行为契约，编译器会在编译期捕获类型错误，而不是在运行时才发现"这个 Agent 的输出格式不对"。
