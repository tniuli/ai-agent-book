...
### 17.2.1 Token 计费：算清楚每一分钱

在生产环境运行 Agent，Token 就是钱。我们先用一个具体例子来算算账。

假设你构建了一个客服 Agent，使用 GPT-5.5 模型，每天处理 1000 次对话，每次对话平均 5 轮交互。我们来算算月成本：
...
# 版本: ch16_model_router.py v1.0
    ...
    MODEL_COSTS = {
        ...
        "gpt-5.5-instant": {"input": 0.50, "output": 1.50},
    }

    def __init__(self):
        self.routing_rules = {
            "simple_qa": "claude-sonnet-4-6",        # 简单问答
            "summarization": "claude-sonnet-4-6",      # 摘要
            "code_generation": "claude-opus-4-7",          # 代码生成
            "complex_reasoning": "claude-opus-4-7",        # 复杂推理
            "tool_calling": "claude-opus-4-7",             # 工具调用
        }
...
| 策略 | 模型 | 输入成本 | 输出成本 | 总计 |
|------|------|----------|----------|------|
| 全部用 GPT-5.5 | GPT-5.5 | 3400×1000×2.5/1M = $8.5 | 1500×1000×10/1M = $15 | **$23.5** |
...
### 17.3.2 LangSmith：LangChain 生态的监控利器
...
def run_agent(query: str) -> str:
    with tracer.start_as_current_span("agent.run") as span:
        ...
        with tracer.start_as_current_span("agent.llm_call") as llm_span:
            ...
            llm_span.set_attribute("llm.model", "claude-opus-4-7")
...
### 17.4.5 LangGraph Platform vs 自建部署
