...
第一步：监督微调（Supervised Fine-Tuning, SFT）。用人工编写的高质量对话数据微调预训练模型，让它学会"按指令行事"的基本格式。
...
Constitutional AI（宪法 AI，CAI）是 Anthropic 提出的对齐方法，其核心思想是：**让模型自己批评自己、自己修正自己**，减少对大量人类标注的依赖。
...
DPO（Direct Preference Optimization，直接偏好优化）由 Rafailov 等人在 2023 年提出，它用一种更简洁的方式绕过了 RLHF 中训练奖励模型和 PPO 优化的复杂流程。
