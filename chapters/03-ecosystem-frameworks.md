...
选择一个 Agent 框架，就像选择一门武学——每门武学都有其核心理念、适用场景和修炼难度。不过在正式介绍之前，有两个常见的误区值得提前提醒：一是"从一而终"，选定一个框架后所有项目都套用它，即使场景不匹配也不愿意换；二是"过度抽象"，用框架的高级 API 封装一切，结果出了问题完全不知道底层发生了什么。更稳妥的做法是根据项目复杂度来选——简单任务用轻量框架，复杂流程再上 LangGraph 这类重量级方案；同时尽量理解框架的底层原理，至少读过源码中的核心循环，保持"随时可以脱离框架"的能力。本节我们将逐一拆解当前最主流的五大 Agent 框架，帮你找到最适合自己的那一门。
