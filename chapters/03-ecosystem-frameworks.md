...
在上一章中，我们理解了 AI Agent 的核心架构和工作原理。现在，是时候走进 AI Agent 的"兵器库"了。框架是你的战甲，模型是你的武艺——战甲决定你能承受多大的冲击，武艺决定你能打出多强的攻击。本章将横向对比 LangGraph、CrewAI、AutoGen、OpenAI Agents SDK 等主流框架的设计哲学和适用场景，掌握 GPT-5.5、Claude、Gemini、开源模型的选型策略，理解不同框架在抽象层级、灵活性、生态成熟度上的权衡。我不会告诉你"哪个最好"——因为最好的，永远是最适合你的。至于工具链与基础设施，我们留给下一章详述。

- - -

## 3.1 框架横评

选择一个 Agent 框架，就像选择一门武学——每门武学都有其核心理念、适用场景和修炼难度。不过在选择之前，有两个误区值得留意：一是"从一而终"，选定一个框架后所有项目都套用它，即使场景不匹配也不愿换；二是"过度抽象"，用框架的高级 API 封装一切，导致出了问题完全不知道底层发生了什么。更稳妥的做法是，根据项目实际复杂度来选型——简单任务用轻量方案，复杂流程再上 LangGraph 这类重量级框架；同时尽量理解框架的底层原理（至少读过核心循环的源码），保持"随时可以脱离框架"的能力。
...
# 配置 LLM
config_list = [{"model": "claude-opus-4-7", "api_key": "your-api-key"}]
...
选型建议：
- 如果你需要构建复杂工作流、需要精确控制每一步执行路径——选 LangGraph
- 如果你更习惯角色分工思维、团队偏产品运营——选 CrewAI
- 如果你做研究实验、喜欢对话式交互——选 AutoGen
- 如果你深度使用 OpenAI 模型、追求简洁稳定——选 OpenAI Agents SDK
- 如果你需要 Agent 自主执行长任务（代码开发、深度研究）、想要开箱即用——选 Deep Agents
...
我们不列参数表格（参数会过时），而是从"什么场景用什么模型"的实战角度出发。
...
OpenAI 的 GPT-5.5 是目前综合能力最均衡的模型之一。它支持文本、图像、音频的多模态输入输出，在推理、代码生成、多语言理解等方面都有出色表现。
...
- **安全性优先：** Anthropic 以宪法 AI（Constitutional AI, CAI）方法论著称，模型输出更安全、更可预测。
- **工具调用：** 支持 Function Calling，兼容主流 Agent 框架。

适用场景： 长文档分析 Agent、需要高安全性的场景、复杂指令遵循、代码审查。

注意事项： 在 Function Calling 的生态成熟度上略逊于 GPT-5.5；国内访问同样需要代理。
...
- **工具调用支持：** Qwen3.6 及以上版本支持 Function Calling，可直接用于 Agent 开发。
...
DeepSeek 以"开源中的性能王者"闻名，其 DeepSeek V4 和 DeepSeek V4 模型在推理能力上接近甚至超越同级闭源模型。

核心优势：
- **推理能力突出：** DeepSeek V4 采用强化学习训练的推理链，在数学、代码和逻辑推理上表现卓越。
...
| 场景 | 首选模型 | 备选模型 | 理由 |
|------|---------|---------|------|
| 通用 Agent 开发 | GPT-5.5 | Claude Opus 4.7 | 生态最成熟，工具调用稳定，幻觉率最低 |
| 长文档分析 | Claude Opus 4.7 | Kimi K2.6 | 200K-1M 上下文，指令遵循好 |
| 视频理解 | Gemini 3.1 Pro | GPT-5.5 | 原生多模态，支持2小时视频理解 |
| 中文场景 | GLM-5.1 / Qwen3.6 | DeepSeek V4 | 中文能力最强，GLM编程能力全球第三 |
| 数学/代码推理 | Claude Opus 4.7 | GLM-5.1 / DeepSeek V4 | SWE-bench Pro 64.3%，GLM达58.4%超GPT-5.5 |
| 长程任务(8小时+) | **GLM-5.1** | Claude Opus 4.7 | 唯一开源模型支持8小时自主工作 |
| 成本敏感 | DeepSeek V4 | Qwen3.6 | API成本仅为GPT-5.5的1/35 |
| 安全合规要求高 | Claude Sonnet 4.6.6 | GPT-5.5 | Constitutional AI，输出可预测 |
| 快速原型验证 | Gemini（免费） | GPT-5.5 Instant (轻量版) | Google AI Studio 免费额度 |
| 端侧部署 | Qwen3.6-7B | Qwen3.6-1.5B | 小规格，可手机端运行 |
| 国产私有化部署 | **GLM-5.1** (MIT开源) | Qwen3.6-72B | 华为云/摩尔线程适配，754B参数MoE架构 |

**GLM-5.1：国产开源模型的里程碑**

智谱AI于2026年4月发布的GLM-5.1是当前国产模型的代表作，在多个维度实现了突破：

- **架构创新**：采用GlmMoeDSA架构，总参数754B（激活40B），支持稀疏注意力机制，推理成本降低20%
- **长程任务能力**：唯一达到8小时级持续工作的开源模型，可自主规划、执行、自我进化并交付工程级成果
- **编程能力**：SWE-bench Pro得分58.4%，超过GPT-5.5和Claude Opus 4.7，位列全球第三、国产第一、开源第一
- **开源许可**：MIT License，支持商用，已上线华为云、摩尔线程等国产算力平台
- **API定价**：$1.40/百万token输入，$4.40/百万token输出，接近Claude Sonnet 4.6.6水平

实际应用中，GLM-5.1展现出惊人的长程工作能力：8小时从零构建完整Linux桌面系统（1200+步操作）、655次迭代优化向量数据库（QPS提升6.9倍）、1000轮工具调用优化机器学习负载。这使其成为需要长时间运行的Agent任务的理想选择。

**一个重要的实战技巧：多模型组合。** 在实际项目中，不必"一棵树上吊死"。常见的策略是：
- **路由模式：** 简单问题用轻量模型（GPT-5.5 Instant (轻量版) / Qwen3.6-7B），复杂问题用旗舰模型（GPT-5.5 / Claude Opus 4.7）。
- **专精模式：** 不同任务用不同模型——代码生成用GLM-5.1或Claude Opus 4.7，文档分析用Kimi K2.6或Claude，通用对话用GPT-5.5。
- **降级模式：** 主模型不可用时自动降级到备用模型，保障服务可用性。
- **成本优化模式：** 核心业务用旗舰模型，边缘场景用DeepSeek V4（成本降低97%）。

**模型选型的"三问法"：** 每次选模型前，问自己三个问题：
1. **我的 Agent 核心任务是什么？** 推理密集还是对话密集？需要多模态还是纯文本？是否需要8小时级长程能力？这决定了模型的"能力门槛"。
2. **我的部署约束是什么？** 必须私有化还是可以用云服务？是否需要国产算力适配？延迟要求是多少？预算是多少？这决定了模型的"部署方式"。
3. **我的风险容忍度是多少？** 能否接受模型供应商的 API 偶尔故障？输出偏差的后果有多严重？是否需要开源可审计？这决定了是否需要多模型降级方案。

### 3.2.4 模型选型的隐性成本

选模型时，大多数人只看"每百万 token 多少钱"和"跑分多少"。但实际项目中，还有几个隐性成本经常被忽略：

**适配成本：** 不同模型的 Function Calling 格式、系统提示词的响应方式、JSON 输出的稳定性都有差异。从 GPT-5.5 切换到 Claude，你可能需要重写提示词、调整工具定义格式、处理不同的错误码。这个适配成本往往被低估——有时候"换模型"的工程量不亚于"重写 Agent"。
...
假设你要为一个中型企业构建"智能客服 Agent"，需要处理售后咨询、订单查询、退换货处理等任务。走一遍选型流程：
...
**第三步：模型筛选。** 数据不出境排除了 GPT-5.5 和 Claude 的云服务。候选方案：Qwen3.6-72B 私有化部署、DeepSeek V4 API（国内可用）、GPT-5.5 via Azure（中国区）。

**第四步：原型验证。** 用三个候选模型各跑 100 条真实客服对话，评估意图分类准确率、工具调用成功率、回复质量。结果可能发现：Qwen3.6-72B 在中文意图理解上最好，但私有化部署的 GPU 成本超出预算；DeepSeek V4 性价比最高，但工具调用偶尔格式不规范；Azure GPT-5.5 最稳定但成本偏高。

**第五步：最终方案。** 采用路由模式——80% 的常规咨询用 DeepSeek V4 处理，20% 的复杂/高风险场景路由到 Azure GPT-5.5。这样既控制了成本，又保障了关键场景的质量。
