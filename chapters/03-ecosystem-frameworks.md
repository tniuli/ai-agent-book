...
> ⚠️ 常见陷阱：框架锁定与过度抽象。选择框架时最容易犯两个错误：一是"从一而终"——选定一个框架后所有项目都用它，即使场景不匹配也不换；二是"过度抽象"——用框架的高级抽象封装一切，导致出了问题完全不知道底层发生了什么。解决方案：根据项目复杂度选框架（简单任务用轻量框架，复杂流程用 LangGraph）；始终理解框架的底层原理（至少读过一遍源码的核心循环）；保持"随时可以脱离框架"的能力。

选择一个 Agent 框架，就像选择一门武学——每门武学都有其核心理念、适用场景和修炼难度。本节我们将逐一拆解当前最主流的五大 Agent 框架，帮你找到最适合自己的那一门。
