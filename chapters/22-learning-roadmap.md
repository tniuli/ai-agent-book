...
| Day 23 | 自动化评估 | 实现 LLM-as-Judge 评估流程，用 Claude Opus 4.7 / GPT-5.5 自动评估 Agent 的输出质量 | 自动评估脚本 |
...
| Day 55 | Agent 微调 | 学习如何用监督微调（Supervised Fine-Tuning, SFT）和 RLHF 优化 Agent 的特定能力 | 微调实验 |
...
**阅读安排：** Ch01-Ch05 快速浏览（1 天），Ch19-Ch22 作为独立阅读材料。核心学习范围：Ch06-Ch18。
...
> **速成版提示：** 速成版节奏很快，确保每天有 1 小时用于消化和整理。不要因为赶进度而跳过动手实践——"看懂了"和"做出来了"之间有很大差距。Ch19-Ch22 可以在 30 天计划结束后作为独立阅读，不需要占用核心学习时间。
...
很多开发者止步于"使用者"——用开源项目写自己的应用，但从不回馈社区。从使用者到贡献者的转变，关键在于转变心态：从"这个工具不好用"到"我怎样才能让它变得更好"。
...
+-------┬------------┬------------┬------------┬------+
        |            |            |            |
+-------┴----+ +-----┴-----+ +---┴----+ +----┴-----+
| Agent      | | Knowledge | | Tool   | | Monitor  |
| Runtime    | | Service   | | Registry| | Service  |
|            | |           | |        | |          |
| - ReAct    | | - RAG     | | - MCP  | | - Traces |
| - Plan     | | - Vector  | | - Custom| | - Metrics|
| - Custom   | | - Chunk   | | - API  | | - Alerts |
+-------┬----+ +-----┬-----+ +---┬----+ +----┬-----+
        |            |            |            |
+-------┴------------┴------------┴------------┴------+
|              Infrastructure Layer                     |
|  +--------+ +----------+ +---------+ +-----------+  |
|  | LLM    | | Vector DB| | RDBMS   | | Cache     |  |
|  | Router | |          | |         | | (Redis)   |  |
|  +--------+ +----------+ +---------+ +-----------+  |
+-----------------------------------------------------+
```
