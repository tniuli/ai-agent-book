...
在动手写代码之前，让我们再深入理解一下 ReAct——因为本章要实现的 Agent，核心就是 ReAct 循环。
...
```mermaid
flowchart TD
    A["1. 开发者注册工具<br/>（名称、描述、参数 schema）"] --> B["2. 用户消息 + 工具列表一起发给 LLM"]
    B --> C{"3. LLM 返回两种可能"}
    C --> D["a) 普通文本回复<br/>（不需要工具）"]
    C --> E["b) tool_calls<br/>（需要调用工具，含工具名和参数）"]
    E --> F["4. 执行工具 → 将结果追加到对话 → 回到第2步"]
```
...
现在，让我们把理论变成代码。在动手之前，先说明两个实现中需要重点防范的问题：ReAct Agent 在运行时容易陷入两种典型故障模式——一是**无限循环**，Agent 反复执行相同或等价的动作（比如反复搜索同一关键词），每轮都判断"还没完成"而继续，直到耗尽步数上限；二是**幻觉工具调用**，LLM 编造不存在的工具名或传入非法参数，导致调用失败，而错误信息又可能进一步误导后续推理。应对策略也很直接：设置硬性最大步数限制（如 10 步）和重复动作检测（连续 3 次相同 action 则强制终止），同时在工具执行层做参数校验，对非法调用返回明确错误而非让异常冒泡。整个实现只需要四个文件，总计不到 200 行 Python 代码。
...
```mermaid
flowchart LR
    ROOT["ch05/minimal_agent/"] --> A["agent.py<br/>ReAct Agent 核心循环"]
    ROOT --> B["tools.py<br/>工具注册与调度<br/>（mock 工具，零配置）"]
    ROOT --> C["memory.py<br/>最小对话历史管理"]
    ROOT --> D["main.py<br/>完整示例入口"]
    ROOT --> E["requirements.txt"]
```
...
在每次循环中记录 Thought、Action、Observation，输出到日志或 LangSmith，让 Agent 的行为可追溯、可调试。这在第 17 章会详细展开。
...
第3轮 Obs:     Python 3.13 预计 2026 年 10 月发布...
