...
现在，让我们把理论变成代码。整个实现只需要四个文件，总计不到 200 行 Python 代码。不过在动手之前，有两个运行时故障模式值得提前了解：一是**无限循环**——Agent 可能反复执行相同或近似的动作（比如换着关键词搜同一个问题），每轮都觉得"还没完成"，直到耗尽步数上限；二是**幻觉工具调用**——LLM 可能编造不存在的工具名或传入不合法参数，调用失败后的错误信息又可能进一步误导后续推理。因此我们的实现中会设置硬性的最大步数限制和重复动作检测机制，同时在工具执行层做参数校验，对异常调用返回明确的错误提示而非让异常直接冒泡。
