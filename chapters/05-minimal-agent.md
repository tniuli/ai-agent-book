...
> ⚠️ 常见陷阱：Agent 无限循环与幻觉工具调用。ReAct Agent 在执行过程中可能陷入两种典型故障模式：一是无限循环——Agent 反复执行相同或等价的动作（如反复搜索同一关键词），每轮都认为"还没完成"，直到耗尽最大步数限制；二是幻觉工具调用——LLM 编造一个不存在的工具名或传入不合法的参数，导致调用失败，而失败后的错误信息又可能进一步误导 Agent。解决方案：设置硬性最大步数限制（如 10 步）和重复动作检测（连续 3 次相同 Action 则强制终止），同时在工具执行层做严格校验，对幻觉调用返回明确的错误提示而非让异常冒泡。

现在，让我们把理论变成代码。整个实现只需要四个文件，总计不到 200 行 Python 代码。
