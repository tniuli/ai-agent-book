...
> ⚠️ 常见陷阱：用 print 调试 Agent 与日志过度/不足。Agent 的多步执行使得 print 调试几乎不可用——输出太多找不到关键信息，输出太少遗漏问题线索。另一个极端是记录所有内容（包括完整的 LLM 响应和工具返回），导致日志膨胀、成本飙升。解决方案：采用三层日志架构（请求级、步骤级、细节级），默认只记录请求级和步骤级，细节级按需开启；所有日志结构化输出（JSON），方便搜索和聚合。
