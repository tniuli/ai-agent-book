...
既然 Agent 调试这么难，我们首先要建立的就是"可观测性"（Observability）。日志追踪是可观测性的基础，也是最朴素、最有效的方法。但 Agent 的多步执行使得传统的 print 调试几乎不可用——输出太多则淹没关键信息，输出太少又遗漏问题线索；走向另一个极端，记录所有内容（包括完整的 LLM 响应和工具返回）又会使日志膨胀、成本飙升。因此需要一套结构化的日志设计：采用三层日志架构（请求级、步骤级、细节级），默认只记录请求级和步骤级，细节级按需开启；所有日志统一以 JSON 格式结构化输出，方便后续搜索和聚合。
