...
```mermaid
flowchart TD
    Trace(["Trace<br/>一次 Agent 请求"])

    Trace --> CE["Run: ChainExecutor"]

    CE --> LLM1["Run: LLM Call<br/>意图识别"]
    LLM1 --> TU1["Token Usage:<br/>input=150, output=30"]

    CE --> TC["Run: Tool Call<br/>查询数据库"]
    TC --> Dur["Duration: 1200ms"]

    CE --> LLM2["Run: LLM Call<br/>生成回答"]
    LLM2 --> TU2["Token Usage:<br/>input=500, output=200"]

    CE --> OP["Run: Output Parser"]
```

> **数据模型对应关系**：Trace = RequestLog（请求级），Run = StepLog（步骤级），Run 的 Attribute = StepLog 的详细字段。父子 Run 关系自然形成调用树，便于定位问题环节。
...
```mermaid
flowchart TD
    Start(["Agent 行为异常"]) --> CheckOutput["查看输出结果"]

    CheckOutput -->|格式错误| P1["提示词问题<br/>输出格式约束不够"]
    CheckOutput -->|内容错误| CheckRun["查看 Run 树"]
    CheckOutput -->|无输出/报错| ToolCheck["检查工具是否正常"]

    CheckRun -->|步骤数过多| ArchIssue["架构问题<br/>死循环或低效路径"]
    CheckRun -->|选错工具| P2["提示词问题<br/>工具选择指导不够"]
    CheckRun -->|工具结果异常| ToolIssue["工具问题"]

    CheckRun --> CheckPrompt["检查 LLM 输入 Prompt"]
    CheckPrompt -->|Prompt 正确但输出错误| ModelIssue["模型问题"]
    CheckPrompt -->|Prompt 缺少关键信息| P3["提示词问题"]
    CheckPrompt -->|Prompt 包含错误信息| Upstream["上游步骤问题"]

    CheckPrompt --> CompareModel["对比不同模型"]
    CompareModel -->|所有模型都有问题| PA["提示词或架构问题"]
    CompareModel -->|只有特定模型有问题| MI["模型问题"]
```

> **排查原则**：古人说"病来如山倒，病去如抽丝"——Agent 出问题往往很突然，但定位需要逐步排查，从输出结果 → Run 树 → Prompt → 模型对比，层层深入。
...
```mermaid
flowchart TD
    Req["用户请求"] --> Tracer["AgentTracer<br/>统一追踪入口"]

    Tracer --> LS["LogStore<br/>日志存储（JSON 文件）"]
    Tracer --> CM["CostMonitor<br/>成本监控"]
    Tracer --> RE["ReplayEngine<br/>回放引擎"]
    Tracer --> Exec["Agent 执行"]
    Exec --> Report(["追踪报告"])
```

> **设计要点**：`AgentTracer` 作为统一入口，内部三个模块各司其职——LogStore 负责三层日志持久化，CostMonitor 实时追踪 Token 消耗并支持预算告警，ReplayEngine 支持从任意步骤回放执行过程。
