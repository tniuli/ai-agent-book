...
到了最激动人心的环节——用 LangChain 重写第 5 章的最小 Agent。我们要做的是同一个东西，但代码会更少、更清晰、更易维护。不过在开始之前，有两个 LCEL 管道开发中最高频的坑值得提前知晓：一是**类型不匹配**——LCEL 用 `|` 串联组件时，上游输出类型必须与下游输入类型兼容，否则运行时报错，最典型的情况是在管道中插入普通函数却忘记用 `RunnableLambda` 包装；二是**模板变量名不一致**——`ChatPromptTemplate` 的变量名如果与代码中传入的 key 对不上，模板渲染时会静默失败而非报错，排查起来非常耗时。建议在搭建管道后用 `chain.get_graph()` 打印 DAG 图来检查类型流转是否正确，同时确保所有自定义函数都经过 `RunnableLambda` 包装。
