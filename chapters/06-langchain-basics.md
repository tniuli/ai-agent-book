...
> ⚠️ 常见陷阱：LCEL 管道中的类型不匹配与 RunnableLambda 遗漏。LCEL 用 `|` 串联组件时，上游的输出类型必须与下游的输入类型兼容，否则运行时报错。最典型的错误是：在管道中插入一个普通函数但忘记用 `RunnableLambda` 包装，导致 `|` 运算符无法识别。另一个常见问题是 `ChatPromptTemplate` 的变量名与代码中的 key 不一致，模板渲染时静默失败。解决方案：先用 `chain.get_graph()` 打印管道的 DAG 图检查类型流转；确保所有自定义函数都用 `RunnableLambda` 包装；模板变量名与代码 key 严格对齐。
