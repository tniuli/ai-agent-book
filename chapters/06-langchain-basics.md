...
上一章我们用纯 Python 手搓了一个最小 Agent，核心代码不到 100 行。你一定发现了：代码虽然不长，但"体力活"很多——手动拼消息列表、手动解析工具调用、手动管理记忆……每加一个功能，就要在好几个地方改代码。LangChain 就是来帮你做这些"体力活"的。它不是神秘的黑盒，而是一套精心设计的抽象层——把你手写的那些模式，变成了可复用的组件。本章将带你掌握 LangChain 的四大核心抽象（Model / Prompt / Chain / Tool），理解 LCEL 管道式编程的设计思想，学会用框架优雅地重写第5章的最小 Agent，并了解 LangChain 生态的扩展方向和常见陷阱。第5章从零实现是"知其然"——理解 Agent 本质；本章学框架是"用其利"——用更少的代码做同样的事。
...
```mermaid
flowchart TB
    A["Prompt<br/>(构造输入)"] --> LCEL
    B["Model<br/>(调用 LLM)"] --> LCEL
    C["Chain<br/>(串联流程)"] --> LCEL
    D["Tool<br/>(扩展能力)"] --> LCEL
    LCEL["LCEL<br/>prompt | llm | parser"]
```
...
```mermaid
flowchart TD
    START["invoke({role, input})"] --> Prompt
    Prompt["Prompt<br/>ChatPromptTemplate<br/><br/>dict → ChatPromptValue<br/>{role, input}<br/>→ [SystemMessage, HumanMessage]"] --> LLM
    LLM["LLM<br/>ChatOpenAI<br/><br/>list[Message] → AIMessage<br/>发给 OpenAI API，返回 AIMessage"] --> Parser
    Parser["Parser<br/>StrOutputParser<br/><br/>AIMessage → str<br/>提取 .content 字段"] --> Result["你好！有什么我可以帮你的吗？"]
```
