...
> ⚠️ 常见陷阱：评估数据泄露与指标误导。评估数据集与训练/提示词数据重叠会导致虚高的评估分数——Agent 不是"能力强"，而是"见过答案"。另一个问题是单一指标误导：仅看"任务完成率"可能忽略 Agent 绕了一大圈才完成（效率低），仅看"工具调用准确率"可能忽略最终答案错误（目标偏移）。解决方案：确保评估数据集与开发数据严格隔离；至少同时跟踪任务完成率、步骤效率、工具准确率三个维度；定期用全新数据做回归测试。

现在让我们把前面学到的所有知识整合起来，搭建一套完整的自动化评测系统。这套系统将包含：数据集管理、多维度评估、报告生成、回归检测等完整功能。
