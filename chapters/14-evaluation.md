...
评估一个 Agent，不能只看"结果对不对"，而需要从多个维度全面衡量。就像评估一个员工，不能只看业绩数字，还要看工作方式、团队协作、职业操守。

### 14.2.1 评估维度全景图

```mermaid
flowchart LR
    subgraph Acc["准确性 Accuracy"]
        A1["事实准确"]
        A2["逻辑一致"]
        A3["任务完成"]
        A4["上下文保持"]
...
> **评估策略**：四个维度缺一不可——准确性决定"对不对"，工具调用决定"会不会用"，效率决定"贵不贵"，安全性决定"危不危险"。
...
运行后，你可以在 LangSmith 控制台看到每个测试用例的得分、评估器评论和详细执行轨迹。这种可视化对于理解 Agent 行为和定位问题非常有帮助。

### 14.3.6 Prompt 效果的量化评估方法
...
SWE-bench 还有一个精简版 SWE-bench Lite，包含 300 个经过筛选的任务，更容易运行。值得注意的是，SWE-bench 是目前对 Agent 最具挑战性的基准测试之一——即使是 GPT-4 配合最强的工具，在完整版 SWE-bench 上的通过率也只有个位数。
...
| **运行难度** | ★★★ | ★★★★ | ★★★★★ |
| **社区活跃度** | ★★★ | ★★★★ | ★★★★★ |
| **代表性模型表现** | GPT-4: ~35% | GPT-4: ~15% | GPT-4: ~4% |

> 注意：以上模型表现为大致参考值，各基准测试会持续更新。GPT-4 在 SWE-bench 上的低分恰恰说明了软件工程任务的极高难度。
...
```mermaid
flowchart LR
    Trigger["触发器<br/>Trigger"] --> Prepare["数据准备<br/>Prepare"]
    Prepare --> Execute["Agent执行<br/>Execute"]
    Execute --> Evaluate["评估计算<br/>Evaluate"]

    Evaluate --> Report["报告生成<br/>Report"]
    Report --> Notify["通知<br/>Notify"]

    Trigger --> CICD["CI/CD 集成"]
```

> **流水线特点**：触发器支持代码变更 / 定时 / 手动 / 数据变更四种模式；评估计算完成后并行生成报告并触发 CI/CD 集成和通知。
...
数据增强是一个容易被忽视但非常有价值的环节。通过同义改写（paraphrasing），你可以用 10 个手工编写的用例自动生成 100 个变体用例，大幅提升评估覆盖率。
...
```mermaid
flowchart TD
    Eval["evaluation/"] --> Datasets["datasets/"]
    Eval --> Results["results/"]
    Eval --> Reports["reports/<br/>生成的报告"]
    Eval --> Config["config.yaml<br/>评测配置"]

    Datasets --> V1["v1_base/<br/>版本1基础数据集"]
    Datasets --> V2["v2_extended/<br/>版本2扩展数据集"]
    Datasets --> Reg["regression/<br/>回归测试数据集"]

    V1 --> QA["qa_cases.json<br/>问答测试用例"]
    V1 --> Tool["tool_cases.json<br/>工具调用测试用例"]
    V1 --> Safety["safety_cases.json<br/>安全性测试用例"]

    Results --> R1["2024-01-15_run1/<br/>每次运行的详细结果"]
    Results --> R2["2024-01-16_run2/"]
```

> **关键实践**：数据集必须版本管理——修改数据集时创建新版本而非覆盖旧版本，确保同一版数据集对比不同 Agent 版本时的公平性。

数据集的版本管理非常重要。当你修改了数据集（比如添加了新的边界用例），应该创建新版本而不是覆盖旧版本。这样你可以用同一版本的数据集对比不同版本的 Agent，保证对比的公平性。
...
现在让我们把前面学到的所有知识整合起来，搭建一套完整的自动化评测系统。动手前有两个问题需要特别注意：一是评估数据集与训练/提示词数据重叠会导致虚高的评估分数——Agent 不是"能力强"，而是"见过答案"，必须确保评估数据集与开发数据严格隔离；二是单一指标具有误导性，仅看"任务完成率"可能忽略 Agent 绕了一大圈才完成的低效问题，仅看"工具调用准确率"可能忽略最终答案错误的目标偏移问题，因此至少需要同时跟踪任务完成率、步骤效率、工具准确率三个维度，并定期用全新数据做回归测试。这套系统将包含：数据集管理、多维度评估、报告生成、回归检测等完整功能。

### 14.6.1 项目结构

```mermaid
flowchart TD
    Root["ch14/"] --> Doc["ch14.md<br/>本章文档"]
    Root --> Req["requirements.txt"]
    Root --> Core["agent_eval_system/<br/>评测系统核心代码"]
    Root --> Sample["sample_agent/<br/>示例Agent"]
    Root --> Data["datasets/<br/>评估数据集"]
    Root --> Entry["run_eval.py<br/>入口脚本"]

    Core --> Init["__init__.py"]
    Core --> Config["config.py<br/>配置管理"]
    Core --> Dataset["dataset.py<br/>数据集管理"]
    Core --> Runner["agent_runner.py<br/>Agent执行器"]
    Core --> Eval["evaluators.py<br/>评估器集合"]
    Core --> Pipe["pipeline.py<br/>评测流水线"]
    Core --> Report["reporter.py<br/>报告生成器"]

    Sample --> SInit["__init__.py"]
    Sample --> Weather["weather_agent.py<br/>天气查询Agent"]

    Data --> WEval["weather_eval.json<br/>天气Agent评测数据"]
```

> **说明**：`agent_eval_system/` 是核心模块，采用可扩展设计——新增评估维度只需实现 Evaluator 类，新增 Agent 只需提供 `run` 函数。
...
让我们直接运行这套评测系统，看看它的完整输出。项目代码已在本章代码目录中提供，你可以直接运行：
