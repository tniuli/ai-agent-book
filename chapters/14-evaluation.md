...
现在让我们把前面学到的所有知识整合起来，搭建一套完整的自动化评测系统。在动手之前有两个坑需要避开：一是**评估数据泄露**——如果你的评测用例和 Agent 的训练数据或 prompt 示例有重叠，高分不代表能力强，只是"见过答案"；二是**单一指标误导**——光看"任务完成率"可能掩盖 Agent 绕了一大圈才完成的效率问题，光看"工具调用准确率"又可能忽略最终答案其实是错的。所以务必保证评估数据集和开发数据严格隔离，同时至少跟踪三个维度（任务完成率、步骤效率、工具准确率），并定期用全新数据做回归验证。这套系统将包含：数据集管理、多维度评估、报告生成、回归检测等完整功能。
