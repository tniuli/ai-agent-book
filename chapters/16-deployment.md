...
> ⚠️ 常见陷阱：流式响应中的背压与连接泄漏。Agent 的流式输出可能持续数十秒，如果客户端在中间断开连接但服务端没有感知，就会导致资源泄漏。另一个常见问题是缓存与一致性：语义缓存在相似但不相同的请求间共享结果，可能返回过时或不精确的答案。解决方案：使用 SSE 时设置心跳包检测连接状态；为流式响应设置合理的超时；缓存设置 TTL 并在关键数据变更时主动失效；对缓存命中率做监控，命中率异常高可能意味着缓存策略过于宽松。

这一节，我们从头到尾搭建一个生产级的 Agent API 服务，包含流式响应、缓存、可观测性、成本追踪等完整功能。
