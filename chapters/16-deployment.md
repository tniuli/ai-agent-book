...
这一节，我们从头到尾搭建一个生产级的 Agent API 服务，包含流式响应、缓存、可观测性、成本追踪等完整功能。在动手之前提醒两个生产环境中容易踩的坑：**流式响应的资源泄漏**——Agent 的流式输出可能持续数十秒，客户端中途断开而服务端没有感知的话，连接和内存就一直挂着不释放，解决方法是 SSE 心跳包检测 + 合理的超时设置；**缓存的一致性陷阱**——语义缓存命中了"差不多"的请求但返回的是过时数据，用户拿到错误信息而不自知，所以缓存必须设 TTL、关键数据变更时主动失效、并且持续监控缓存命中率（异常偏高往往说明策略过于宽松）。
