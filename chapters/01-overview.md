...
flowchart TB
    subgraph A[" "]
        direction TB
        P["感知输入<br/>（用户指令）"] --> B["大脑（LLM）<br/>理解 / 推理 / 规划"]
        B --> M["记忆<br/>短期/长期"]
        B --> T["工具调用<br/>搜索/代码/API"]
        M --> ACT["行动输出<br/>执行实际操作"]
        T --> ACT
    end
```
...
```mermaid
flowchart TD
    A["1950 图灵测试提出"] --> B["1966 ELIZA（最早的聊天程序）"]
    B --> C["1970s MYCIN 等专家系统"]
    C --> D["1995 Agent 理论框架<br/>（Wooldridge & Jennings）"]
    D --> E["1997 IBM 深蓝击败卡斯帕罗夫"]
    E --> F["2011 IBM Watson 赢得 Jeopardy!"]
    F --> G["2016 AlphaGo 击败李世石"]
    G --> H["2017 Transformer 架构论文<br/>（Attention Is All You Need）"]
    H --> I["2018 GPT-1 发布；<br/>OpenAI Five 击败 Dota 2 职业战队"]
    I --> J["2019 GPT-2 发布"]
    J --> K["2020 GPT-3 发布，<br/>展示 In-Context Learning 能力"]
    K --> L["2022 ChatGPT 发布（11月）"]
    L --> M["2023 Agent 元年：<br/>ReAct、Toolformer、AutoGPT、HuggingGPT<br/>─── LLM Agent 时代开启 ───"]
    M --> N["2024 多 Agent 协作框架涌现<br/>（CrewAI、AutoGen 等）<br/>函数调用成为主流范式"]
    N --> O["2025 Agent 进入工程化阶段<br/>MCP 等标准化协议出现"]
    O --> P["▼ 2026 ? 未来可期"]
```
...
flowchart TD
    START["目标"] --> TASK["任务拆解"]
    TASK --> NEXT["选择下一步行动"]
    NEXT --> EXEC["执行行动<br/>（搜索/读写文件/执行代码）"]
    EXEC --> OBS["观察结果"]
    OBS --> EVAL["评估进展"]
    EVAL -->|"未完成"| TASK
    EVAL -->|"完成"| DONE["完成"]
