...
打个比方：传统软件像是手电筒，你按开关它就亮，不按就不亮；聊天机器人像是对讲机，你说一句它回一句；而 Agent 更像是一个实习生——你交代一个任务，他自己去查资料、做分析、写报告，只在关键节点来问你一声。
...
注意这里发生了什么：模型不是直接回答"北京天气如何"，而是主动判断"我需要调用天气工具"，并自动提取了参数 `city: "北京"`。这就是 Agent 的雏形——它能根据目标，自主选择使用什么工具。
...
打个比方：RPA 像是一个照着菜谱一板一眼做菜的厨师，菜谱上写"加盐3克"，它就加3克，即使这批食材特别咸需要少加盐，它也不会变通。

### 1.2.4 传统自动化：只能"走流程"

传统自动化脚本是程序员写的 if-else 逻辑。比如一个 CI/CD 流水线：代码提交 -> 运行测试 -> 测试通过则部署，失败则通知。

这种自动化的特点是确定性：给定相同的输入，一定得到相同的输出。这对于标准化流程非常有效，但面对模糊、多变、需要判断的场景就力不从心了。

传统自动化像是一条流水线，每个环节都是固定的，产品必须按固定规格来。如果你需要处理的是非标准化的工作——比如分析一份措辞暧昧的合同、回复一封语气微妙的客户邮件——流水线就派不上用场了。

### 1.2.5 Agent：能"想"也能"做"

Agent 与上面三者的本质区别在于两个字：自主。

Agent 不是一个"输入-输出"的黑盒，而是一个循环决策系统：
...
关键区别在于：Agent 在每一步都在思考下一步该做什么，而不是执行预设的固定流程。如果它发现投诉记录里文本格式混乱，它会调整分析策略；如果某个热点特别突出，它会深入挖掘原因。这种适应性是其他三种系统不具备的。
...
Agent 的概念虽然很早就存在，但它真正"能用"是 2023-2025 年的事。
...
1950  图灵测试提出
  ...
  |   1970s  MYCIN 等专家系统
  |
  |   1995  Agent 理论框架（Wooldridge & Jennings）
  |
  |   1997  IBM 深蓝击败卡斯帕罗夫
  |
  |   2011  IBM Watson 赢得 Jeopardy!
  |
  |   2016  AlphaGo 击败李世石
  |
  |   2017  Transformer 架构论文（Attention Is All You Need）
  |
  |   2018  GPT-1 发布；OpenAI Five 击败 Dota 2 职业战队
  |
  |   2019  GPT-2 发布
  |
  |   2020  GPT-3 发布，展示 In-Context Learning 能力
  |
  |   2022  ChatGPT 发布（11月）
  |
  |   2023  Agent 元年：ReAct、Toolformer、AutoGPT、HuggingGPT
  |        ---- LLM Agent 时代开启 ----
  |
  |   2024  多 Agent 协作框架涌现（CrewAI、AutoGen 等）
  |        函数调用（Function Calling）成为主流范式
  |
  |   2025  Agent 进入工程化阶段
  |        MCP（Model Context Protocol）等标准化协议出现
  |        Agent 在企业级场景开始落地
  |
  v   2026+ ? 持续演进
```
...
谈论 Agent 的能力边界时，成本是一个经常被忽略但关键的维度。
...
> 📌 **Prompt Engineering 融入点**：ReAct 的 Thought-Action-Observation 格式本质上是一种**提示词设计模式**。你不需要修改模型架构——只需要在提示词中定义清晰的"思考-行动-观察"格式，引导 LLM 按这个结构输出，就能驱动 Agent 的推理循环。这揭示了 Prompt Engineering 在 Agent 开发中的核心地位：提示词不仅是"指令"，更是 Agent 的"行为规范文档"。
...
1. **动手搭建一个最小 Agent**：使用 OpenAI API（或任意支持函数调用（Function Calling）的 LLM API），实现一个能调用至少 2 个工具的 Agent。建议工具：天气查询 + 简单计算器。关键体验：观察模型如何自主决定调用哪个工具、提取什么参数。提示：可以用 `python-dotenv` 管理 API Key，不要把 Key 硬编码在代码里。
...
3. Richards, T. "AutoGPT: An Autonomous GPT-4 Experiment." GitHub, 2023. https://github.com/Significant-Gravitas/AutoGPT
...
7. Weizenbaum, J. "ELIZA—A Computer Program For the Study of Natural Language Communication Between Man and Machine." *Communications of the ACM*, 1966.
8. Shortliffe, E. *Computer-Based Medical Consultations: MYCIN*. Elsevier, 1976.
9. Wooldridge, M. and Jennings, N. "Intelligent Agents: Theory and Practice." *Knowledge Engineering Review*, 1995.
...
12. Brown, T. et al. "Language Models are Few-Shot Learners." NeurIPS 2020. arXiv:2005.14165
13. Berner, C. et al. "Dota 2 with Large Scale Deep Reinforcement Learning." arXiv:1912.06680, 2019.
