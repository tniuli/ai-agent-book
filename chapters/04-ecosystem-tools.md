...
在上一章中，我们完成了框架与模型的选型——骨架已成，大脑已就。但一个真正的 Agent，还需要"双手"来操控外部工具，需要"粮草"来存储长期记忆，需要"情报系统"来观测自身行为。工具链与基础设施，正是这三者的统称。Function Calling 和 MCP 协议赋予 Agent 与世界交互的能力，向量数据库为 Agent 提供持久化的知识记忆，可观测性工具让 Agent 的运行状态透明可控。它们不是"锦上添花"，而是 Agent 从"能对话"走向"能做事"的关键基础设施。
...
```python
# Function Calling 示例（OpenAI 兼容协议）
import os
from openai import OpenAI

# 本书使用 OpenAI 兼容协议调用多种模型
# 设置 OPENAI_BASE_URL 指向你的 API 端点，OPENAI_API_KEY 为对应的 Key
client = OpenAI(
...
| 模型 | 支持状态 | 特点 |
|------|---------|------|
| GPT-5.5 | 原生支持 | 最成熟，支持并行调用 |
| Claude Opus 4.7 | 原生支持 | 稳定，支持复杂参数 |
| Gemini 3.1 Pro | 原生支持 | 支持，但偶有格式偏差 |
| Qwen3.6 | 支持 | 原生支持，效果优秀 |
| DeepSeek V4 | 支持 | 支持完善，性价比高 |
...
+-----------------+                  +-----------------+
       |                                    |
       | 可以同时连接多个 Server              | 可以被多个 Client 连接
       v                                    v
+-----------------+                  +-----------------+
...
3. Johnson, J. et al. "Billion-scale similarity search with GPUs." IEEE T-BD 2021. arXiv:1702.08734
