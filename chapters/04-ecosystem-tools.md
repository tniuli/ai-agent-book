...
```mermaid
flowchart TD
    A["用户输入"] --> B{"LLM 判断是否需要调用工具"}
    B -->|"需要"| C["生成函数调用指令<br/>（函数名 + 参数）"]
    C --> D["执行层调用实际函数"]
    D --> E["将函数返回结果拼入上下文"]
    E --> F["LLM 基于新上下文继续生成"]
    B -->|"不需要"| G["直接生成文本回复"]
...
```mermaid
flowchart LR
    Client["Agent 应用<br/>（MCP Client）"] <-->|"MCP 协议"| S1["MCP Server<br/>文件系统工具"]
    Client <-->|"MCP 协议"| S2["MCP Server<br/>数据库工具"]

    NOTE1["可以同时连接多个 Server"] -.-> Client
    NOTE2["可被多个 Client 连接"] -.-> S2
```
