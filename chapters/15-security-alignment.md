...
flowchart TB
    subgraph L1["第一层：输入防护"]
        direction TB
        L1D["输入验证 + Prompt 注入检测 + 速率限制"]
    end
    subgraph L2["第二层：运行时防护"]
        direction TB
        L2D["权限控制 + 行为约束 + 异常检测"]
    end
    subgraph L3["第三层：输出防护"]
        direction TB
        L3D["输出审查 + 审计日志"]
    end
```
...
```mermaid
flowchart TB
    subgraph PermArch["权限模型架构"]
        direction TB
        RoleDef["角色定义<br/>(Who)"] -->|"定义"| Pol["权限策略<br/>(What)"]
        Pol -->|"控制"| ResAcc["资源访问<br/>(Which)"]
        RoleDef --> RoleHier["角色层级<br/>viewer / editor / admin"]
        Pol --> OpConst["操作约束<br/>只读 / 读写 / 全部"]
        ResAcc --> ResLevel["资源分级<br/>public / internal / secret"]
        Engine["动态权限评估引擎<br/>上下文 + 角色 + 操作 + 资源 → 允许/拒绝"]
    end
```
...
```mermaid
flowchart TB
    PreTrain["预训练模型"] -->|"SFT"| SFTModel["SFT模型"]
    SFTModel --> HumanData["人类偏好数据"]
    SFTModel --> Candidates["生成多个候选回复"]
    HumanData --> RMTrain["奖励模型训练"]
    Candidates --> RankLabel["人类排序标注"]
    RankLabel --> RM
...
```mermaid
flowchart TB
    HarmQ["有害提问"] --> HarmResp["模型生成有害回复"]
    HarmResp --> SelfCrit["依据宪法自我批评"]
    SelfCrit --> FixedResp["生成修正回复"]
    FixedResp -->|"用作 SFT 训练数据"| SL_CAI["SL-CAI 阶段"]

    TwoReply["两个候选回复"] --> AIPref["模型依据宪法判断偏好<br/>生成 AI 偏好数据"]
    AIPref --> TrainRM2["训练奖励模型"]
    TrainRM2 --> RLHF2["RLHF"]
```
...
```mermaid
flowchart LR
    subgraph RLHF_Path["RLHF 路径"]
        direction LR
        PD1["偏好数据"] --> RM["训练奖励模型"] --> PPO["PPO 优化策略<br/>复杂，不稳定"]
    end
    subgraph DPO_Path["DPO 路径"]
        direction LR
        PD2["偏好数据"] --> DPO["直接优化策略<br/>简单，稳定"]
    end
```
...
```mermaid
flowchart TB
    UserReq["用户请求"] --> IG["输入安全网关<br/>InputGuard<br/>输入验证 + Prompt 注入检测"]
    IG -->|"安全"| PG["权限守卫<br/>PermGuard<br/>最小权限检查"]
    PG -->|"允许"| AC["Agent 核心<br/>AgentCore<br/>LLM + 工具调用"]
    AC -->|"原始输出"| OG["输出过滤器<br/>OutputGuard<br/>敏感信息脱敏 + 内容审查"]
    OG -->|"安全输出"| AL["审计日志<br/>AuditLogger<br/>全流程记录"]
    AL --> Resp["安全响应"]
```
