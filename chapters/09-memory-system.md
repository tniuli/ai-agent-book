...
不过需要提前说明的是，长期记忆的语义检索并非万能——当用户的问题与存储内容的语义距离较远时（比如用户问"推荐晚餐"但存储的是"喜欢吃辣"），检索可能返回完全无关的记忆条目，反而干扰 LLM 的判断。更危险的是上下文窗口溢出：检索返回的文档片段过多或过长，加上对话历史后超出 LLM 的窗口限制，Agent 会直接截断甚至崩溃。所以在实际系统中，需要为检索结果设置相似度阈值（低于阈值的直接丢弃），限制 Top-K 数量和单条记忆的最大长度，并在拼接上下文前计算总 token 数，超出预算时按优先级裁剪。
