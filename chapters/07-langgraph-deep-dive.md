...
> ⚠️ 常见陷阱：条件边的路由逻辑遗漏与 State 字段缺失。LangGraph 的条件边（conditional edge）依赖一个路由函数返回下一个节点名，如果路由函数的返回值没有对应的边，运行时会抛出 KeyError。另一个常见问题是 State 中缺少某个字段，导致节点函数读取时 KeyError。解决方案：路由函数的所有可能返回值都必须在 `add_conditional_edges` 中有对应的映射；State 定义时预留所有可能用到的字段，用 `Optional` 标注可能为空的字段。
